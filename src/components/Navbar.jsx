...
export default function Navbar({
  ...
  justify-between pt-5 pl-6 pb-14 md:pb-0 absolute md:relative z-10
  transform transition-transform duration-500
  ${isOpenNavbar ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        <a
          href="#"
      ...
      <button className=" fixed md:hidden mb-4 p-2 left-4 bottom-4 text-4xl text-blue-400 z-50">
        {isOpenNavbar ? (
          <i className="bx bx-x" onClick={HandleNavbar}></i>
        ) : (
          <i className="bx bx-menu" onClick={HandleNavbar}></i>
        )}
      </button>
    </>
  );
